...
export class HomePageService {
  ...
  getProducts():Observable<IProduct[]>{
    let tempVar = this.http.get<IProduct[]>('https://qkbackend.azurewebsites.net/api/home/getproducts')
    console.log(tempVar)
    return tempVar
  }
  ...
  ValidateUser(userEmailID:string, userPassword:string, type:string):Observable<number>
  {
    ...
    let result=this.http.post<number>('https://qkfunction.azurewebsites.net/api/LoginFunction?code=hYL_gxChWY6V4kVghX5SeOb_frdfhZ5G0ND7Bn8Jqck0AzFu4mauVA==',user)
    return result
